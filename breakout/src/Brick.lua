...
function Brick:init(x, y)
    ...
    -- used to determine whether this brick is locked; default is no
    self.locked = false
...
function Brick:render()
    -- render special skin for locked bricks; locked bricks are inherently in play
    if self.locked then
        love.graphics.draw(gTextures['main'], gFrames['bricks'][24], self.x, self.y)
    elseif self.inPlay and not self.locked then
...
--[[
    Unlocks any brick that is locked; triggered when a key is captured. Also sets locked
    brick flag to false, so no more key powerups are initiated. Change 'color' of brick to 
    unlocked brick.
]]
function unlockBrick(bricks)
