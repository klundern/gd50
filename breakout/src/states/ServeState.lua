...
function ServeState:enter(params)
    ...
    -- init a table to hold our various balls; choose a random skin (for fun)
    -- first update call will have the ball track the paddle
    self.balls = GenerateBallTable(
        math.random(7),                                 -- choose a random skin to begin with
        self.paddle.x + (self.paddle.width / 2 - 4),    -- centered on paddle
        self.paddle.y - 8,                              -- centered on paddle
        0, 0)                                           -- both dx and dy are 0 initially
end

function ServeState:update(dt)
    -- have the paddle track the player
    self.paddle:update(dt)

    -- have the ball (there should only be one) track the paddle
