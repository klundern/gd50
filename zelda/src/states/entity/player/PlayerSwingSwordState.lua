...
function PlayerSwingSwordState:update(dt)
    ...
    for k, entity in pairs(self.dungeon.currentRoom.entities) do
        if entity:collides(self.swordHitbox) then
            ...
            -- random chance to drop a heart when the entity takes damage
            if math.random(120) == 1 then
                -- play a sound
                gSounds['heart-drop']:play()

                -- initialize heart object at x and y of entity is spawned from
                local heart = GameObject(GAME_OBJECT_DEFS['heart'], entity.x, entity.y)

                -- when heart is consumed, add to player's health (max of 6)
                ...
                -- add heart to table of objects in the Room
